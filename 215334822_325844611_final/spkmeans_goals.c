...
int build_T_of_spectral_kmeans(size_t K, matrix_t* output) {
	...
	if (matrix_new(jacobi_res.eigen_vectors.rows, jacobi_res.eigen_vectors.cols, output)) goto error;
	...
	for (i = 0; i < output->rows; i++) {
		...
		for (j = 0; j < output->cols; j++) {
			matrix_set(*output, i, j, matrix_get(jacobi_res.eigen_vectors, i, j) / norm_of_row );
		}
	}
